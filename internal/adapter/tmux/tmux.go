...
// CloseTab kills the window that holds the pane, with every pane in it. A
// pane id is unique across the server, so the instance is not needed; a
// window linked into other sessions leaves them all.
func (h *Host) CloseTab(ctx context.Context, _ revier.TargetRef, panel revier.PanelID) error {
	_, err := h.run(ctx, "kill-window", "-t", panel.String())
	return err
}
