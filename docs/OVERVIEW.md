...
| `internal/adapter/<tool>/` | One tool each. No policy. `kitty` and `tmux` (runtime), `gnome` (window), `proc` (what a link on another machine started here), `claude`, `opencode`, and `execprobe` (probes), `ssh` (remote). |
